...
impl State {
    pub fn workspace_list(&self) -> Vec<Workspace> {
        let expected_output = self.expected_output.as_ref();
        self.workspace_state
            .workspace_groups()
            .filter(|g| {
                expected_output
                    .map_or(true, |expected| g.outputs.iter().any(|o| Some(o) == Some(expected)))
            })
            .flat_map(|g| {
                g.workspaces
                    .iter()
...
impl OutputHandler for State {
    ...
    fn new_output(
        ...
        let should_set_expected = if self.configured_output.is_empty() {
            self.expected_output.is_none()
        } else {
            info.name.as_deref() == Some(&self.configured_output)
        };

        if should_set_expected {
